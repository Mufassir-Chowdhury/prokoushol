<script lang="ts">
    ...
    import type { Option, Subject } from '../+layout';
    
    export let data: PageData;
    const department = $page.params.dept;
    
    let selected: Option | undefined = data.options[0];
    let selected_course: Subject | undefined;
    let isMobileRailsOpen = false;

    $: currentSemesterCourses = data.subjects;
...
<div class="flex h-full w-full relative">
    ...
    <!-- Rails Container (conditionally shown on mobile) -->
    <div class="{isMobileRailsOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 absolute md:relative z-20 flex transition-transform duration-300 ease-in-out">
        <!-- First Rail: Semesters -->
        <div class="w-48 md:w-60 bg-gray-100 h-full flex flex-col shrink-0">
            <div class="p-4 border-b border-gray-200">
                <div class="font-bold text-gray-800 mb-1">{data.departments[department]}</div>
                <div class="text-sm text-gray-600">Select Semester</div>
            </div>
            <div class="flex flex-col space-y-1 p-2 overflow-y-auto">
                {#each data.options as option}
                    <button 
                        class="p-2 text-left rounded transition-colors duration-100 {selected === option ? 'bg-blue-100 text-blue-800' : 'hover:bg-gray-200'}"
        ...
        <!-- Second Rail: Courses -->
        <div class="w-48 md:w-64 bg-gray-50 border-l border-r border-gray-200 h-full flex flex-col shrink-0">
            <div class="p-4 font-semibold text-gray-700">Courses</div>
            <div class="flex flex-col space-y-1 p-2 overflow-y-auto">
                {#each currentSemesterCourses as course}
                    <button 
    ...
    <div class="flex-1 bg-white h-full overflow-y-auto">
        {#if selected && selected_course}
            <div class="p-4 md:p-6">
                <h2 class="text-xl md:text-2xl font-bold mb-4 md:mb-6">{selected_course.name} - {selected.text}</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                    {#each data.cards as card}
                        <a 
                            href={`/${department}/${selected.value}/${selected_course.value}/${card.value}`}
                            class="block"
                        >
        ...
        {:else}
            <div class="flex items-center justify-center h-full text-gray-500 p-4 text-center">
                Select a semester and course to view content
            </div>
        {/if}
    </div>
</div>